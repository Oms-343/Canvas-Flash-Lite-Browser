...
const App: React.FC = () => {
  ...
  const currentPage =
    activeTab.currentIndex >= 0
      ? activeTab.history[activeTab.currentIndex]
      : null;
  ...
  const handleUpdateContent = useCallback(
    (newHtml: string) => {
      updateTab(activeTabIndex, (tab) => ({
        ...tab,
        generatedContent: newHtml,
        history: tab.history.map((page, i) =>
          i === tab.currentIndex ? { ...page, html: newHtml } : page,
        ),
      }));
    },
  ...
  const handleToggleEditor = useCallback(() => {
    updateTab(activeTabIndex, (tab) => ({
      ...tab,
      isEditingCode: !tab.isEditingCode,
    }));
  }, [activeTabIndex, updateTab]);

  const handleExportHtml = useCallback(() => {
    if (!activeTab.generatedContent) return;

    const blob = new Blob([activeTab.generatedContent], { type: "text/html" });
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `${activeTab.breadcrumb.sitename || "website"}.html`;
  ...
  return (
    <OuterFrame tokenCount={activeTab.tokenCount} isLoading={activeTab.loading}>
      <BrowserShell
      ...
      >
        {isNewTab ? (
          ...
          <CodeEditor
            code={activeTab.generatedContent}
